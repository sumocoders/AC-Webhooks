...
class WebhooksAdminController extends AdminController
{
    ...
    function index()
    {
        ...
        if ($this->request->isSubmitted()) {
            try {
                foreach (Webhooks::getEvents() as $eventname) {
                    $webhooks_data['events'][$eventname]['enabled'] = ($this->request->post(
                                                                           'enable_' . $eventname,
                                                                           0
                                                                       ) == 1);
                    ...
                    if ($webhooks_data['events'][$eventname]['enabled']) {
                        ...
                        if (!filter_var(
                            ...
                            throw new Exception('invalid callback for ' . $eventname);
                        } // store valid callback
                        else {
                            $webhooks_data['events'][$eventname]['callback'] = $callback;
                        }
                    }
                }
